...
namespace Axis.Pollux.ABAC
{
    public static class Extensions
    {
        ...
        public static sigma.Policy.PolicySet ToSigma(this pollux.PolicySet policySet, IPolicyExpressionProvider pep)
        {
            ...
            else return new sigma.Policy.PolicySet
            {
                ...
                SubPolicies = policySet.Policies.Select(ps => ps.ToSigma(pep)),
                SubSets = policySet.SubSets.Select(ps => ps.ToSigma(pep)),
                Target = new sigma.Policy.PolicyTarget { Condition = pep.ToTargetExpression(policySet.TargetExpression) }
            };
        }
        public static sigma.Policy.Policy ToSigma(this pollux.Policy policy, IPolicyExpressionProvider pep)
        {
            ...
            else return new sigma.Policy.Policy
            {
                ...
                Rules = policy.Rules.Select(r => r.ToSigma(pep)),
                Target = new sigma.Policy.PolicyTarget { Condition = pep.ToTargetExpression(policy.TargetExpression) }
            };
        }
        public static sigma.Policy.Rule ToSigma(this pollux.Rule rule, IPolicyExpressionProvider pep)
        {
            if (rule == null) return null;
            else return new sigma.Policy.Rule
            {
                Id = rule.Name,
                ActionCondition = new sigma.ActionExpression { Expression = pep.ToAttributeExpression<sigma.Action>(rule.ActionCondition) },
                EnvironmentCondition = new sigma.EnvironmentExpression { Expression = pep.ToAttributeExpression<sigma.Environment>(rule.ActionCondition) },
                ResourceCondition = new sigma.ResourceExpression { Expression = pep.ToAttributeExpression<sigma.Resource>(rule.ActionCondition) },
                SubjectCondition = new sigma.SubjectExpression { Expression = pep.ToAttributeExpression<sigma.Subject>(rule.ActionCondition) }
            };
        }
        ...
        //{
        //    return new pollux.PolicySet
        //    {
        //        CombinationClause = policySet.CombinationClause.AsString(),
        //        Name = policySet.Id,
        //        Policies = policySet.SubPolicies.Select(p => p.ToPollux()).ToList(),
        //        SubSets = policySet.SubSets.Select(p => p.ToPollux()).ToList(),
        //        Title = policySet.Title,
        //        TargetExpression = policySet.
        //    }
        //}
        ...
        #region Combination Clause Conversions
